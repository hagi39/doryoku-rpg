...
(function (global) {
  ...
  const MAX = {
    ...
    EXAMPLES: 2,     // 例題
    PITFALLS: 3,     // つまずきやすい点
    TITLE: 60,      // 数式入りの見出しは長くなりやすいので広めに取る
    BODY: 400,
  };
  ...
  const TEXT_RULES = [
    ...
    '図・表・グラフは画面に出せないので、「下の図」のように指さず、必要なことは言葉と式で書く。',
    '高校の範囲を超える知識は使わない。',
  ...
  function str(v, max) {
    ...
    // 切るときは切ったと分かるようにする(文の途中でぷつりと終わると不具合に見えるため)
  ...
  async function generate(app, skill, opts) {
    ...
    const user = [
      ...
      '  title は30字くらいまでの短い見出しにする。長い数式は title に入れず body に書く。',
