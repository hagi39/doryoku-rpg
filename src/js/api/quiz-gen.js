...
(function (global) {
  ...
  const FORMAT_RULES = [
    ...
    '選択肢どうしが数学的に同値にならないようにする。任意定数の置き方が違うだけの式(例: (A+Bn)×2^(n-1) と (A+Bn)×2^n)は、どちらも正解になるので並べない。',
    ...
    '問題文だけで解けるように書く。図・表・グラフは画面に出せないので、「下の図」「次の表」のように問題文の外にあるものを指さない。必要な情報はすべて問題文の中に言葉と式で書く。',
    '問題文を「次のようになります。」「以下のとおりです。」のような、続きを約束する言い方で終わらせない。問題文は、何を答えるのかを問う文で終える。',
    '一つの問題の中で使う文字(x, a, n など)は、問題文・選択肢・解説を通して同じものを使う。同じものを指すのに途中で別の文字に変えない。',
    '「aをxとして読み替えてください」「言い換えると」のように、問題文の中で設定を言い直したり説明し直したりしない。書いている途中で文字がずれたら、後から言い足して直すのではなく、初めから一つの文字で問題文を書き直す。',
    'explanation は正解の理由を1〜2文で書く。',
    'explanation で「選択肢0は〜」のように番号で選択肢に触れるときは、その番号の選択肢の中身を実際に読み直してから書く。番号と中身がずれた説明は書かない。自信がなければ、番号を使わずに式や言葉でそのまま指し示す。',
    '高校の範囲を超える知識は使わない。',
  ];
